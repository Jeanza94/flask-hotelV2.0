...
@app.route('/administrarUsuarios', methods=['POST'])
def adminUser():
    ...
    try:
        ...
        conn = sql.connect('hotel-database.db')
        cursor = conn.cursor()
        buscarUsuario = f'SELECT * FROM usuarios where email="{email}"'
        cursor.execute(buscarUsuario)
        usuario = cursor.fetchall()
        if len(usuario)>0 :
            eliminar = f'DELETE FROM usuarios where email="{email}"'
            cursor.execute(eliminar)
            print('email')
            conn.commit()
            conn.close()
            return '<h1>felicidades loquita</h1>'
        return '<h1>el usuario no existe</h1>'
    except:
        print('error')
    return '<h1>No se pudo eliminar el dato</h1>'
...
def administrar():
    if 'user' in session:
        conn = sql.connect('hotel-database.db')
        cursor = conn.cursor()
        buscarUsuarios = f'SELECT name, email, cargo FROM usuarios'
        cursor.execute(buscarUsuarios)
        usuarios = cursor.fetchall()
        print(usuarios)

        buscarHabitaciones = f'SELECT * FROM habitaciones'
        cursor.execute(buscarHabitaciones)
        habitaciones = cursor.fetchall()
        print(habitaciones)
        return render_template('administrar.html', usuarios=usuarios, habitaciones = habitaciones)
